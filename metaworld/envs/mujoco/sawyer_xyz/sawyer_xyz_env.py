...
class SawyerXYZEnv(SawyerMocapBase, metaclass=abc.ABCMeta):
    ...
    def _get_obs(self):
        ...
        if "image" in self._observation_mode:
            camera_names = [
                "corner3",
                "corner",
                "corner2",
            ...
            img_obs = {
                f"{camera_name}_rgb": self.sim.render(
                    height=300, width=300, camera_name=camera_name
                )
    ...
    def step(self, action):
        ...
        reward, info = self.evaluate_state(self._last_stable_obs["state"], action)
